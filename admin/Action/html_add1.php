...
if($HtmlID==""){
      $q=mysql_query("INSERT INTO `html` (`Title`,`ImgAltText`,`SmallImage`,`BigImage`,`SmallDetails`,`BigDetails`,`SortIndex`,`Status`,`LastUpdated`) VALUES ('$Title','$ImgAltText','$resize_image','$BigImage','$SmallDetails','$BigDetails','$SortIndex','$Status',CURDATE())")or die (mysql_error ());

            echo "<script>window.location='../html_list.php?&&mssg=Inserted Successfully';</script>";   
	 }	

else{
	
	 $q=mysql_query("UPDATE `html` SET `HtmlID`='$HtmlID',`Title`='$Title',`SmallImage`='$resize_image',`BigImage`='$BigImage',`ImgAltText`='$ImgAltText',`SmallDetails`='$SmallDetails',`BigDetails`='$BigDetails',`SortIndex`='$SortIndex' ,`Status`='$Status',`LastUpdated`=CURDATE() WHERE `HtmlID`='$HtmlID'")or die (mysql_error ());

            
		   if($flagpop!=1){
         echo "<script>window.location='../html_list.php?&HtmlID=$HtmlID&mssg=Updated Successfully';</script>";   
        }
        else{
            echo "<script>window.location='../Frontend/html_frontadd.php?menuId=$menuId&HtmlID=$HtmlID&mssg=Updated Successfully';</script>";
        }
