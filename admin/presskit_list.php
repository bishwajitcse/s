...
if($_GET['Task']=="delete")
{
 $q=mysql_query("delete from `presskit` WHERE `PressKitID`='$PressKitID'")or die (mysql_error ());
...
?>
  ...
  <div class="site_performance_top"><span class="site_text">USER </span> LIST     <div class="add_new"><a href="presskit_add.php"><img width="" height="" alt="" src="assets/image/add_new.png"></a></div>
 ...
 <div class="gallery_table">
	...
	<? if ($mssg!=""){  echo '<div class=submission_mssg>' .$mssg. '</div>' ; }?>
	 ...
	 <? $sql_page = "SELECT COUNT(*) as cnt FROM `presskit`";
...
<table cellspacing="0" border="0" cellpadding="4" width="100%" style="border-top:1px solid; border-bottom:1px solid; text-align:center" id="ctl00_ContentPlaceHolder1_GridView1" rules="all" class="gallery_table_list">
    ...
    $i=1;
$sql = "SELECT * FROM presskit order by PressKitID DESC LIMIT $offset ,$rowsperpage";
...
?>
		...
		<td><?php  echo $row['SortIndex'];?></td>
		
			
<td><a href="presskitdata_list.php?PressKitID=<? echo $row['PressKitID']?>" style="text-align:center;">Press Kit List</a></td>

				<td><a href="presskit_add.php?PressKitID=<? echo $row['PressKitID']?>" style="text-align:center;"><img src="Assets/image/edit.png" /></a></td>
					<td><a href="presskit_list.php?PressKitID=<? echo $row['PressKitID'] ?>&Task=delete" onClick='return confirmSubmit()'><img src="Assets/image/delete.png" /></a></td>
