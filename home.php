	  ...
	  <?
        ...
        <?   echo (" <img src=Admin/Upload/".$row[BigImage]." width='' height=''/>"); ?>
          <span class="banner-text">
            ...
            <span class="desc"><? echo $row["SmallDetails"];?>		
			
			
				  </span>
				  
				  <?  if($adminflag!=0){
              echo "<div class=editbutton><a target='_blank' onclick=MM_openBrWindow('admin/Frontend/topbanner_frontadd.php?menuId=" . $MenuID ."&CID=".$CID."&BannerID=".$row_id.               "','TEXTEDITOR','scrollbars=yes,resizable=yes,width=850,height=700')><img src='admin/Assets/image/EditeMode.jpg'></a></div>";
                  }?></span> </li><? } ?>
...
<!-- Two Boxes -->
<section class="row home-boxes main">
  <div class="container">


<?
		$sql_html = mysql_query("SELECT html.* FROM menulink INNER JOIN html ON MenuLink.LinkID = html.HtmlID WHERE MenuID=" . $MenuID . " and LinkWith = 'html'") or die(mysql_error());
		$overview = "";
        $x = 0;
		while($row_html = mysql_fetch_array($sql_html)){  
			$x++;
			$flag3=$row_html[HtmlID];
			if ($x == 3){
      ...
      <div class="box box-1"> 
        ...
        <div class="title">
          ...
          <a href="malta.html">Enter</a>
		    <?php
                 if($adminflag!=0){
                        echo "<div class=editbutton><a target='_blank' onclick=MM_openBrWindow('admin/Frontend/html_frontadd.php?menuId=" . $MenuID."&HtmlID=" . $row_html[HtmlID] ."','TEXTEDITOR','scrollbars=yes,resizable=yes,width=850,height=700')><img src='admin/Assets/image/EditeMode.jpg'></a></div>";
                 }
            ?>
        </div>
      </div>
...
<section class="row">
    ...
    <div class="col-64">
      <div class="box">
        <? echo $overview; ?>
		 <?php
             if($adminflag!=0){
                 echo "<div class=editbutton><a target='_blank' onclick=MM_openBrWindow('admin/Frontend/html_frontadd.php?menuId=" . $MenuID."&HtmlID=" . $flag3 ."','TEXTEDITOR','scrollbars=yes,resizable=yes,width=850,height=700')><img src='admin/Assets/image/EditeMode.jpg'></a></div>";
             }
        ?>
      </div>
    </div>
      ...
      <div class="news-box box">
        ...
        <div class="content">
          <ul>
		   ...
		   <?
		$sql_news = mysql_query("SELECT news.* FROM menulink INNER JOIN news ON MenuLink.LinkID = news.NewsID WHERE  LinkWith = 'news' and type='news'") or die(mysql_error());
 ...
 <li>
			 ...
			 echo  $row_news['SmallDetails'];
			   ...
			   <?php
                  if($adminflag!=0){
                      echo "<a target='_blank' onclick=MM_openBrWindow('admin/Frontend/newsadd_fornthand.php?menuId=" . $MenuID."&NewsID=" . $row_news[NewsID] ."&flagpop=1','TEXTEDITOR','scrollbars=yes,resizable=yes,width=850,height=700')><img src='admin/Assets/image/EditeMode.jpg'></a>";
                  }
                  ?>
            </li>
